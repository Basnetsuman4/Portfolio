...
const SpaceBackground: React.FC = () => {
	...
	useEffect(() => {
		...
		const createRocket = () => {
			...
			const baseSpeed = 1 + Math.random() * 0.6;
		...
		const drawRocket = (r: typeof rocket) => {
			...
			const fireSize = 8 + speedRatio * 26;
			const firePulse = Math.sin(Date.now() * 0.08) * speedRatio;
			...
			ctx.beginPath();
			ctx.moveTo(-3, 10);
			ctx.lineTo(0, 10 + fireSize + firePulse * 4);
			ctx.lineTo(3, 10);
			ctx.fill();
		...
		const update = () => {
			...
			rocket.stateTimer--;
			if (rocket.stateTimer <= 0) {
				if (Math.random() < 0.6) {
					rocket.targetSpeed = rocket.minSpeed + Math.random() * 0.25;
				} else {
					rocket.targetSpeed = 1.2 + Math.random() * (rocket.maxSpeed - 1.2);
				}
				rocket.stateTimer = 160 + Math.random() * 260;
			}

			rocket.speed += (rocket.targetSpeed - rocket.speed) * rocket.accel;

			/* -------- REALISTIC TURNING -------- */

			rocket.turnTimer--;
			if (rocket.turnTimer <= 0) {
				rocket.desiredTurn = (Math.random() - 0.5) * rocket.maxTurnRate;
				rocket.turnTimer = 160 + Math.random() * 260;
			}

			rocket.turnVelocity +=
				(rocket.desiredTurn - rocket.turnVelocity) * rocket.turnAccel;

			rocket.desiredTurn *= rocket.turnDecay;
			rocket.angle += rocket.turnVelocity;
			...
			if (
				rocket.x < -limit ||
				rocket.x > canvas.width + limit ||
				rocket.y < -limit ||
				rocket.y > canvas.height + limit
			) {
				rocket = createRocket();
			}
			...
			>
				...
				<div
					className={`absolute w-5 h-5 border border-cyber-cyan/50 transition-all duration-300 ${
						isHovering ? "border-cyber-cyan" : "border-cyber-cyan/40"
					}`}
				/>
		...
		<div className="relative flex items-start group mb-32 last:mb-0">
			...
			<div className="flex-1 pb-12">
				<div className="inline-flex items-center gap-4 mb-4">
					<span className="text-[10px] font-mono text-cyber-cyan/40 font-bold tracking-widest">
						TYPE_ROLE
					</span>
				...
				<div className="flex flex-wrap items-center gap-6 mb-8">
					...
					<span className="lg:hidden text-[10px] text-cyber-cyan/40 font-bold">
						{exp.period}
					</span>
				</div>
		...
		return (
			<div className="fixed inset-0 bg-pitch-black z-[100] flex items-center justify-center">
				<div className="flex flex-col items-center gap-6 relative">
					...
					<div className="text-[10px] tracking-[1.2em] text-cyber-cyan animate-pulse font-extrabold uppercase">
						Initializing_System
					</div>
				</div>
					...
					>
						<div className="relative w-10 h-10 flex items-center justify-center">
							...
							<div className="relative z-10 flex items-baseline">
								<span className="font-heading font-black text-cyber-cyan text-lg tracking-tighter">
									S
								</span>
								<span className="font-heading font-black text-white text-lg tracking-tighter opacity-80">
									B
								</span>
							</div>
					...
					<section className="min-h-screen flex flex-col justify-center px-10 md:px-24 lg:px-44 relative">
						<div className="max-w-6xl w-full">
							...
							<h1 className="font-heading text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[0.9] mb-12 text-white animate-slide-up [animation-delay:200ms]">
								Building <br />{" "}
								<span className="text-cyber-cyan">Resilient UIs.</span>
							</h1>
							<p className="text-lg md:text-xl font-light leading-relaxed max-w-2xl text-slate-400 animate-slide-up [animation-delay:400ms]">
								User-centric interfaces for SaaS and PropTech platforms,
								emphasizing scalable state management and high-fidelity design
								systems that turn complexity into clarity.
							</p>

							<div className="mt-20 flex flex-wrap gap-8 animate-slide-up [animation-delay:600ms]">
								{["NEXTJS", "REACT", "STYLED_COMPONENTS", "FORMIK"].map(
									(item) => (
										<div
											key={item}
											className="px-10 py-4 glass-morphism rounded-full text-[10px] tracking-[0.4em] font-black text-cyber-cyan border-cyber-cyan/20 hover:border-cyber-cyan/60 transition-all hover:-translate-y-2"
										>
											{item}
										</div>
									)
					...
					<Section id="work" title="Industrial Archives">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
							{PROJECTS.map((p) => (
								<div
									key={p.id}
									className="transform hover:-translate-y-4 transition-all duration-1000"
								>
									<WorkCard project={p} />
								</div>
							))}
					...
					<Section id="education" title="Academic Foundation">
						<div className="grid grid-cols-1 lg:grid-cols-2 gap-10 py-10">
							{EDUCATION.map((edu, i) => (
								<div
									key={i}
									className="p-10 glass-morphism rounded-[3rem] border-white/5 hover:border-cyber-cyan/20 transition-all group relative overflow-hidden"
								>
					...
					<Section id="skills" title="Tech Capabilities">
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 py-10">
							{SKILLS.map((skillGroup, idx) => (
								<div
									key={idx}
									className="p-8 glass-morphism rounded-[2.5rem] border-white/5 hover:border-cyber-cyan/20 transition-all group"
								>
									...
									<ul className="space-y-4">
										{skillGroup.items.map((skill, sIdx) => (
											<li
												key={sIdx}
												className="text-[13px] text-slate-500 group-hover:text-slate-300 transition-colors flex items-center gap-3"
											>
						...
						<div className="py-24 text-center glass-morphism rounded-[6rem] border-cyber-cyan/10 p-16 md:p-32 relative overflow-hidden group">
							...
							<div className="flex flex-col items-center gap-6 relative z-10">
								<a
									...
									className="inline-block text-xl md:text-3xl font-heading text-white hover:text-cyber-cyan transition-all duration-700 py-8 px-16 glass-morphism rounded-full border-cyber-cyan/20 hover:scale-105 shadow-[0_0_50px_rgba(0,245,255,0.05)]"
								>
							...
							<div className="mt-24 flex justify-center flex-wrap gap-12 md:gap-24 relative z-10">
								...
								>
									<svg
										className="w-5 h-5 transition-all duration-500 group-hover/social:scale-110 group-hover/social:drop-shadow-[0_0_8px_#00F5FF]"
										viewBox="0 0 24 24"
										fill="currentColor"
									>
